...
class SchedulerPPMixin:
    ...
    def event_loop_pp(self):
        ...
        ###luo 原始是 last_rank_comm_queue: deque[Tuple[torch.cuda.Event, PPProxyTensors]] = deque()
        ...
        while True:
            ...
            for mb_id in range(loop_size):
                ...
                if self.cur_batch:
                    ...
                    ###luo 原始是 event = torch.cuda.Event()
                    ###luo 原始是 event.record(torch.cuda.current_stream()) 
                    event = torch.get_device_module(self.device).Event()
                    event.record(torch.get_device_module(self.device).current_stream())

                ###luo 
                with torch.get_device_module(self.device).stream(self.pp_process_stream):
                # (last rank) send the outputs to the next step
                    if self.pp_group.is_last_rank:
                        ...
                        if mbs[next_first_rank_mb_id] is not None:
                            ...
                            ###luo 原始是 torch.cuda.current_stream().wait_event(q_event)
